...
class TurboClone::BroadcastableTest < ActionCable::Channel::TestCase
  ...
  test "#broadcast_prepend_to" do
    article = Article.create! content: "Fake"
    assert_broadcast_on "stream", turbo_stream_action_tag(:prepend, target: 'fake', template: render(article)) do
      article.broadcast_prepend_to "stream", target: 'fake'
    end
  end
  ...
  test "#broadcast_prepend_later_to" do
    ...
    assert_broadcast_on "stream", turbo_stream_action_tag(:prepend, target: 'fake', template: render(article)) do
      perform_enqueued_jobs do
        article.broadcast_prepend_later_to "stream", target: 'fake'
      end
    end
  end
